...
fn main() -> anyhow::Result<()> {
...

    {
        ...
        for i in iter_serde_data() {
            writer.write(&serde_json::to_vec_pretty(&i)?)?;
        }
        writer.flush()?;
    }

    {
        ...
        let deserializer = Deserializer::from_reader(reader).into_iter::<SerdeAggregate>();
        for agg in deserializer {
            println!("{:#?}", agg);
        }
    }
...
// use this for serde because of string key
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
pub struct SerdeAggregator {
    aggregate: HashMap<String, AggregateData>, // Key is MD5 hash of page url
}

impl From<Aggregator> for SerdeAggregator {
